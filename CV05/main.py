...
def calculate_accuracy_and_fscore(ground_truth, detected: list):
    ...
    for i in range(len(ground_truth)):
        if ground_truth[i] == 0 and detected[i][0] == 0 or ground_truth[i] == 1 and detected[i][0] == 1:
            ...
            fp += 1
            # print(f'False positive detected at index {i}')
        elif ground_truth[i] == 1 and detected[i][0] == 0:
            fn += 1
            # print(f'False negative detected at index {i}')
...
def load_data_from_folders(occupied_folder: str, empty_folder: str):
    full_images = glob.glob(occupied_folder + "/*.png")
    free_images = glob.glob(empty_folder + "/*.png")
    
    images = full_images + free_images
    labels = [1] * len(full_images) + [0] * len(free_images)
...
def lbp_detection(test_images: list, pkm_coordinates: list, ground_truth_data: list):
    ...
    for config in lbp_configs:
        time = datetime.now()
        scores = []
        print(f"Processing LBP configuration: radius={config['radius']}, neighbors={config['neighbors']}")
        
        recognizer = cv2.face.LBPHFaceRecognizer.create(radius=config['radius'], neighbors=config['neighbors'])
        ...
        for img_name, ground_truth in zip(test_images, ground_truth_data):
            ...
            for coord in pkm_coordinates:
                ...
                img_gray = cv2.cvtColor(one_place_img, cv2.COLOR_BGR2GRAY)
